...
# Create reverse mapping: State -> Director
STATE_TO_DIRECTOR = {}
...
def get_director_for_state(state_code):
    """Get director name for a given state code"""
    return STATE_TO_DIRECTOR.get(state_code, None)
...
def get_banker_locations(active_bankers, branch_data, portfolio_centroids):
    ...
    # Combine both - include COVERAGE and DIRECTOR_NAME columns
    banker_locations = pd.concat([
        in_market[['PORT_CODE', 'BANKER_TYPE', 'ROLE_TYPE', 'LAT', 'LON', 'COVERAGE', 'DIRECTOR_NAME']],
        centralized[['PORT_CODE', 'BANKER_TYPE', 'ROLE_TYPE', 'LAT', 'LON', 'COVERAGE', 'DIRECTOR_NAME']]
    ])
...
def assign_by_state(customers_no_coords, active_bankers):
    """Assign customers without coordinates based on state coverage - filtered by director"""
    
    if len(customers_no_coords) == 0:
        return pd.DataFrame(columns=['CG_ECN', 'HH_ECN', 'PORT_CODE', 'PROM_SEG_RAW', 'BANKER_TYPE', 'DIRECTOR_NAME']), pd.DataFrame()
    
    bankers = active_bankers.copy()
    
    if len(bankers) == 0:
        return pd.DataFrame(columns=['CG_ECN', 'HH_ECN', 'PORT_CODE', 'PROM_SEG_RAW', 'BANKER_TYPE', 'DIRECTOR_NAME']), customers_no_coords
    ...
    for _, customer in customers_no_coords.iterrows():
        ...
        # Get the director for this state
        required_director = get_director_for_state(customer_state_code)
        
        if not required_director:
            unmatched_customers.append(customer)
            unmatched_states.add(customer_state_code)
            continue
        
        # Find bankers covering this state AND under the correct director
        matching_bankers = bankers_by_state[
            (bankers_by_state['STATE'] == customer_state_code) & 
            (bankers_by_state['DIRECTOR_NAME'] == required_director)
        ]
        
        if len(matching_bankers) > 0:
            # Assign to first matching banker
...
def assign_customers(unassigned_pop, banker_locations, in_market_radius=40, centralized_radius=600):
    """Assign customers to nearest bankers within radius - filtered by director"""
    
    customers = unassigned_pop.copy()
    
    if len(customers) == 0:
        empty_df = pd.DataFrame(columns=['CG_ECN', 'HH_ECN', 'PORT_CODE', 'PROM_SEG_RAW', 'BANKER_TYPE', 'DIRECTOR_NAME'])
        return empty_df, pd.DataFrame()
    
    # Add customer state code and director
    customers['CUSTOMER_STATE_CODE'] = customers['BILLINGSTATE'].apply(convert_state_name_to_code)
    customers['CUSTOMER_DIRECTOR'] = customers['CUSTOMER_STATE_CODE'].apply(get_director_for_state)
    ...
    if len(customers_with_coords) > 0 and len(bankers) > 0:
        ...
        # Process each director's customers separately
        for director in DIRECTOR_STATES.keys():
            director_customers = customers_with_coords[customers_with_coords['CUSTOMER_DIRECTOR'] == director].copy()
            
            if len(director_customers) == 0:
                continue
            
            print(f"\n  Processing {len(director_customers)} customers for director: {director}")
            
            # Filter bankers to only those under this director
            director_bankers = bankers[bankers['DIRECTOR_NAME'] == director].copy()
            
            if len(director_bankers) == 0:
                print(f"    Warning: No bankers found for director {director}")
                unassigned_with_coords.extend(director_customers.to_dict('records'))
                continue
            
            # Step 1: Try IN MARKET bankers first (40 miles)
            in_market_bankers = director_bankers[director_bankers['ROLE_TYPE'] == 'IN MARKET'].copy()
            ...
            if len(in_market_bankers) > 0:
                ...
                customer_coords = director_customers[['ECN_LAT', 'ECN_LON']].values.astype(float)
                ...
                for idx in assigned_indices:
                    ...
                    assignments.append({
                        'CG_ECN': director_customers.iloc[idx]['CG_ECN'],
                        'HH_ECN': director_customers.iloc[idx]['HH_ECN'],
                        'PORT_CODE': in_market_bankers.iloc[banker_idx]['PORT_CODE'],
                        'PROM_SEG_RAW': director_customers.iloc[idx]['PROM_SEG_RAW'],
                        'BANKER_TYPE': in_market_bankers.iloc[banker_idx]['BANKER_TYPE'],
                        'DIRECTOR_NAME': director
                    })
                
                # Remove assigned customers
                director_customers = director_customers.iloc[~mask].copy()
            
            # Step 2: Assign remaining customers to CENTRALIZED bankers (600 miles)
            if len(director_customers) > 0:
                centralized_bankers = director_bankers[director_bankers['ROLE_TYPE'] == 'CENTRALIZED'].copy()
                ...
                if len(centralized_bankers) > 0:
                    ...
                    for idx in np.where(unassigned_mask)[0]:
                        unassigned_with_coords.append(director_customers.iloc[idx])
                else:
                    unassigned_with_coords.extend(director_customers.to_dict('records'))
    
    assignments_df = pd.DataFrame(assignments) if assignments else pd.DataFrame(
        columns=['CG_ECN', 'HH_ECN', 'PORT_CODE', 'PROM_SEG_RAW', 'BANKER_TYPE', 'DIRECTOR_NAME']
    )
...
def assign_randomly(customers_unassigned, active_bankers):
    """Randomly assign remaining unassigned customers - filtered by director"""
    
    if len(customers_unassigned) == 0:
        return pd.DataFrame(columns=['CG_ECN', 'HH_ECN', 'PORT_CODE', 'PROM_SEG_RAW', 'BANKER_TYPE', 'DIRECTOR_NAME'])
    
    # Add customer state code and director
    customers = customers_unassigned.copy()
    customers['CUSTOMER_STATE_CODE'] = customers['BILLINGSTATE'].apply(convert_state_name_to_code)
    customers['CUSTOMER_DIRECTOR'] = customers['CUSTOMER_STATE_CODE'].apply(get_director_for_state)
    ...
    for _, customer in customers.iterrows():
        required_director = customer['CUSTOMER_DIRECTOR']
        
        if not required_director:
            unassigned.append(customer)
            continue
        
        # Filter bankers to only those under the correct director
        director_bankers = bankers[bankers['DIRECTOR_NAME'] == required_director].copy()
        
        if len(director_bankers) == 0:
            print(f"  Warning: No bankers found for director {required_director}")
            unassigned.append(customer)
            continue
        
        # Get unique PORT_CODEs with their BANKER_TYPE for this director
        portfolio_banker_map = director_bankers[['PORT_CODE', 'BANKER_TYPE', 'DIRECTOR_NAME']].drop_duplicates()
        ...
        assignments.append({
            ...
            'DIRECTOR_NAME': random_portfolio_row['DIRECTOR_NAME']
        })
    
    if unassigned:
        print(f"  Warning: {len(unassigned)} customers could not be assigned (no director match)")
...
def main(df1, df2, df3, df4, df5):
    ...
    print("="*60)
    print("CUSTOMER-BANKER ASSIGNMENT PROCESS (WITH DIRECTOR ALIGNMENT)")
    print("="*60)
    ...
    # Get BANKER_TYPE and DIRECTOR_NAME from df2 based on PORT_CODE
    backfill_assignments = backfill_assignments.merge(
        df2[['PORT_CODE', 'BANKER_TYPE', 'DIRECTOR_NAME']].drop_duplicates(),
        on='PORT_CODE',
        how='left'
    )
    ...
    for director in DIRECTOR_STATES.keys():
        director_count = len(banker_locations[banker_locations['DIRECTOR_NAME'] == director])
        print(f"    {director}: {director_count} bankers")
    ...
    print(f"\n{'='*60}")
    print("STEP 1: Assigning customers geographically (director-filtered)")
    print(f"{'='*60}")
    ...
    print(f"\n{'='*60}")
    print("STEP 2: Assigning remaining customers by state (director-filtered)")
    print(f"{'='*60}")
    ...
    print(f"\n{'='*60}")
    print("STEP 3: Randomly assigning remaining customers (director-filtered)")
    print(f"{'='*60}")
    ...
    all_assignments = [
        backfill_assignments[['CG_ECN', 'HH_ECN', 'PORT_CODE', 'PROM_SEG_RAW', 'BANKER_TYPE', 'DIRECTOR_NAME']],
    ...
    if len(final_assignments) > 0:
        director_counts = final_assignments['DIRECTOR_NAME'].value_counts()
    ...
    print(f"\nCross-tabulation (Director vs Banker Type):")
    if len(final_assignments) > 0:
        crosstab_dir = pd.crosstab(final_assignments['DIRECTOR_NAME'], 
                                    final_assignments['BANKER_TYPE'], 
                                    margins=True)
        print(crosstab_dir)
    ...
    return final_assignments[['CG_ECN', 'HH_ECN', 'PORT_CODE', 'PROM_SEG_RAW', 'BANKER_TYPE', 'DIRECTOR_NAME', 'ECN_LAT', 'ECN_LON']]
