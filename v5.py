...
def create_customers_only_map(all_portfolios, branch_data):
    """Create a map showing only customers"""
...
def create_au_only_map(all_portfolios, branch_data):
    """Create a map showing only AU locations"""
    ...
    print(f"DEBUG AU MAP: au_locations count: {len(au_locations)}")
    print(f"DEBUG AU MAP: au_locations content: {au_locations}")
    ...
    # Calculate center point from AU locations
    if au_locations:
        center_lat = sum(lat for _, lat, _ in au_locations) / len(au_locations)
        center_lon = sum(lon for _, _, lon in au_locations) / len(au_locations)
        zoom = 6
    else:
        center_lat = 39.8283
        center_lon = -98.5795
        ...
        with col2:
            ...
            if preview_portfolios:
                ...
                with col_a:
                    ...
                    customers_only_map = create_customers_only_map(preview_portfolios, branch_data)
                    if customers_only_map:
                        st.plotly_chart(customers_only_map, use_container_width=True)
                
                with col_b:
                    st.subheader("AU Map")
                    au_only_map = create_au_only_map(preview_portfolios, branch_data)
                    if au_only_map:
                        st.plotly_chart(au_only_map, use_container_width=True)
            else:
