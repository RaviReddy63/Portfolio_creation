...
def check_state_director_match(customer_state_code, director_name):
    """
    Check if a customer's state code is in the director's allowed states.
    
    Args:
        customer_state_code: 2-letter state code (e.g., 'CA')
        director_name: Director's name (e.g., 'MOJGAN MADADI')
    
    Returns:
        True if match is valid, False otherwise
    """
    if pd.isna(customer_state_code) or pd.isna(director_name):
        return False
    
    director_upper = str(director_name).strip().upper()
    allowed_states = DIRECTOR_STATES.get(director_upper, [])
    
    return customer_state_code in allowed_states
...
def create_portfolio_location_map(rbrm_data, branch_data, portfolio_centroids):
    """
    Create a mapping of portfolio code to location (lat, lon) and director.
    ...
    for _, row in rbrm_data.iterrows():
        ...
        director = row.get('DIRECTOR', None)  # Get director name
...
def calculate_portfolio_requirements(hh_df, portfolio_locations):
    ...
    Now counts BOTH Segment 3 and Segment 4 for all portfolios.
...
def trim_oversized_portfolios(hh_df, portfolio_stats):
    ...
    Now trims from BOTH Segment 3 and Segment 4.
    ...
    for portfolio_cd, stats in portfolio_stats.items():
        if stats['excess'] > 0:
            # Get all households of BOTH segments in this portfolio
            portfolio_hhs = hh_df[
...
def optimize_in_market_portfolios(hh_df, portfolio_stats, placement='IN MARKET'):
    """
    Optimize portfolios using iterative radius expansion.
    Assigns each household to nearest undersized portfolio.
    Now assigns BOTH Segment 3 and Segment 4 customers to BOTH RM and RC portfolios.
    Includes state-director matching for new assignments.
    Uses portfolio_locations BallTree created once.
    """
    hh_df = hh_df.copy()
    
    # Filter portfolios by placement (IN MARKET or CENTRALIZED)
    portfolios = [
    ...
    # Set radius parameters based on placement
    if placement == 'IN MARKET':
    ...
    for radius in range(start_radius, max_radius + 1, increment):
        ...
        portfolio_stats = calculate_portfolio_requirements(hh_df, 
            {p: {'lat': portfolio_stats[p]['lat'], 
                 ...
                 'director': portfolio_stats[p].get('director', None)} 
        ...
        # Add state code column for matching
        ...
        # For each household, assign to nearest undersized portfolio with state match
        for hh_idx, (indices, distances) in zip(unassigned_hhs.index, zip(indices_list, distances_list)):
            ...
            # Find nearest undersized portfolio with state-director match
            assigned = False
            for i, dist in zip(indices, distances):
                ...
                director_name = portfolio_stats[portfolio_cd].get('director', None)
                
                # Check state-director match
                if not check_state_director_match(customer_state_code, director_name):
        ...
        print(f"  Skipped {skipped_state_mismatch} assignments due to state-director mismatch")
...
def fill_portfolios_to_max(hh_df, portfolio_stats, placement='IN MARKET', radius=20):
    """
    ...
    # Filter portfolios by placement
    portfolios = [
        p for p, s in portfolio_stats.items() 
        if s['placement'] == placement
    ]
    ...
    # For each household, assign to nearest portfolio with capacity and state match
    for hh_idx, (indices, distances) in zip(unassigned_hhs.index, zip(indices_list, distances_list)):
        if len(indices) == 0:
            continue
        ...
        # Find nearest portfolio with capacity and state-director match
        assigned = False
        for i, dist in zip(indices, distances):
...
def assign_remaining_households(hh_df, portfolio_stats, placement='CENTRALIZED'):
    """
    Final cleanup: Assign all remaining unassigned households.
    Now handles BOTH Segment 3 and Segment 4 together for BOTH RM and RC portfolios.
    Includes state-director matching for new assignments.
    ...
    if len(remaining) > 0 and len(portfolios) > 0:
        ...
        # For each household, assign to nearest portfolio that is below MAX with state match
        for hh_idx, (indices, distances) in zip(remaining.index, zip(indices_list, distances_list)):
            ...
            # Try to find nearest portfolio that is below MAX with state-director match
            assigned = False
            for idx_pos, (i, dist) in enumerate(zip(indices, distances)):
                nearest_portfolio = portfolios[i]
                director_name = portfolio_stats[nearest_portfolio].get('director', None)
        ...
        if not_assigned_count > 0:
            print(f"    Could not assign {not_assigned_count} households (all nearest portfolios at MAX or state mismatch)")
    ...
    if len(remaining) > 0:
        ...
        # Get original MAX value (use RM max as baseline, will handle RC separately)
        ...
        while len(remaining) > 0 and iteration <= max_iterations:
            # Increase MAX by 20 for both RM and RC
            ...
            # For each household, assign to nearest portfolio below new MAX with state match
            for hh_idx, (indices, distances) in zip(remaining.index, zip(indices_list, distances_list)):
                if len(indices) == 0:
                    continue
                ...
                # Try to find nearest portfolio that is below new MAX with state-director match
                assigned = False
                for i, dist in zip(indices, distances):
                    nearest_portfolio = portfolios[i]
                    ...
                    # Get appropriate max based on banker type
            ...
            # Add state code column for next iteration
            if len(remaining) > 0:
                remaining['STATE_CODE'] = remaining['BILLINGSTATE'].apply(convert_state_to_code)
...
def run_portfolio_reconstruction(hh_df, branch_data, rbrm_data, sbb_data, portfolio_centroids):
    ...
    Now assigns BOTH Segment 3 and Segment 4 to both RM and RC portfolios.
    Uses unified functions for both banker types.
    Includes state-director matching for new assignments.
    """
    print("="*70)
    ...
    hh_df = optimize_in_market_portfolios(hh_df, portfolio_stats, placement='IN MARKET')
    
    # ========== STEP 4: FILL IN MARKET TO MAX (RM & RC together) ==========
    print("\n[STEP 4: FILL IN MARKET TO MAX (RM & RC)]")
    portfolio_stats = calculate_portfolio_requirements(hh_df, portfolio_locations)
    hh_df = fill_portfolios_to_max(hh_df, portfolio_stats, placement='IN MARKET', radius=20)
    
    # ========== STEP 5: CENTRALIZED OPTIMIZATION (RM & RC together) ==========
    print("\n[STEP 5: CENTRALIZED OPTIMIZATION (RM & RC)]")
    portfolio_stats = calculate_portfolio_requirements(hh_df, portfolio_locations)
    hh_df = optimize_in_market_portfolios(hh_df, portfolio_stats, placement='CENTRALIZED')
    
    # ========== STEP 6: FINAL CLEANUP (RM & RC together) ==========
    print("\n[STEP 6: FINAL CLEANUP (RM & RC)]")
    portfolio_stats = calculate_portfolio_requirements(hh_df, portfolio_locations)
    hh_df = assign_remaining_households(hh_df, portfolio_stats, placement='CENTRALIZED')
    ...
    unassigned_seg3_4 = hh_df[(hh_df['NEW_SEGMENT'].isin([3, 4])) & (hh_df['RULE'] == 'POOL') & (hh_df['CG_PORTFOLIO_CD'].isna())]
